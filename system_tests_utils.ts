...
// eslint-disable-next-line import/no-extraneous-dependencies
import { Personal } from '../web3.js/packages/web3-eth-personal';

// eslint-disable-next-line import/no-extraneous-dependencies
import {
	create as createAccount,
} from '../web3.js/packages/web3-eth-accounts';
// eslint-disable-next-line import/no-extraneous-dependencies
...
export const getSystemTestBackend = (): string => getEnvVar('WEB3_SYSTEM_TEST_BACKEND') ?? 'geth';
	...
	new Promise<void>((resolve, reject) => {
		...
		const interval = setInterval(() => {
			if (currentAttempt > maxNumberOfAttempts - 1) {
				...
				(web3Context.provider as unknown as any).getStatus() === status
			) {
				clearInterval(interval);
				resolve();
			}
...
export const closeOpenConnection = async (web3Context:any) => {
	if (!getSystemTestProvider().startsWith('ws')) {
		return;
	}
	...
	if (
		...
		(web3Context.provider as unknown as any).getStatus() === 'connecting'
	) {
	...
	if (
		web3Context?.provider &&
		'disconnect' in (web3Context.provider as unknown as any)
	) {
		(web3Context.provider as unknown as any).disconnect(1000, '');
	}
};
